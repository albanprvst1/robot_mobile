...
class Mur(Obstacle):
    ...
    def dessiner(self, vue):
        p1, p2 = vue.convertir_coordonnees(self.x1, self.y1), vue.convertir_coordonnees(self.x2, self.y2)
...
class ZoneEau(Obstacle):
    ...
    def collision(self, xr, yr, rr):
        return (self.x-self.w/2 <= xr <= self.x+self.w/2 and self.y-self.h/2 <= yr <= self.y+self.h/2)
    def dessiner(self, vue):
...
class ZoneSafe:
    def __init__(self, x, y, nom): self.x, self.y, self.nom = x, y, nom
    def dessiner(self, vue):
        px, py = vue.convertir_coordonnees(self.x, self.y)
        s = int(0.8 * vue.scale)
        pygame.draw.rect(vue.screen, (200, 180, 150), (px - s//2, py - s//2, s, s))
        pygame.draw.polygon(vue.screen, (150, 0, 0), [(px-s//2-4, py-s//2), (px+s//2+4, py-s//2), (px, py-s)])
...
class Garde:
    def __init__(self, x, y):
        self.x, self.y = x, y
        self.vx, self.vy = random.choice([-1.5, 1.5]), random.choice([-1.5, 1.5])
...
class Environnement:
    ...
    @property
    def est_nuit(self): return self.heure < 6 or self.heure > 20
